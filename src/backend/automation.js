...
export async function run(event) {
  ...
  if (!rules.length) { await kvs.delete(suggestionKey(issueKey)); return; }
  ...
  if (!rule) { await kvs.delete(suggestionKey(issueKey)); return; }
  const approvers = [];
  const seen = new Set();
  for (const configured of (Array.isArray(rule.approvers) ? rule.approvers : []).slice(0, 20)) {
    ...
    try {
      ...
      if (canonical?.accountId && canonical.active !== false) approvers.push({ accountId: canonical.accountId, displayName: clean(canonical.displayName, 200) });
    } catch (error) { console.warn('Smart Approval: unable to resolve suggested approver', error?.message || error); }
  }
  if (!approvers.length) { await kvs.delete(suggestionKey(issueKey)); return; }
  await kvs.set(suggestionKey(issueKey), {
    issueKey, projectId, ruleId: clean(rule.id || rule.name, 200), ruleName: clean(rule.name, 200), triggerStatus: clean(rule.triggerStatus, 200), approvers,
    approvalMode: rule.approvalMode === 'any' ? 'any' : 'all', message: clean(rule.message || '', 2000),
    reminderHours: Math.min(720, Math.max(1, Number(rule.reminderHours || settings.reminderHours || 24))),
    pendingTargetStatus: clean(rule.pendingTargetStatus || settings.pendingTargetStatus, 200), approveTargetStatus: clean(rule.approveTargetStatus || settings.approveTargetStatus, 200), declineTargetStatus: clean(rule.declineTargetStatus || settings.declineTargetStatus, 200),
    pendingTransitionId: clean(rule.pendingTransitionId || settings.pendingTransitionId, 100), approveTransitionId: clean(rule.approveTransitionId || settings.approveTransitionId, 100), declineTransitionId: clean(rule.declineTransitionId || settings.declineTransitionId, 100),
